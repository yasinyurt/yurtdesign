<div class="container-fluid mt-4">
    ...
    <p class="mb-4">Yeni bir blog yazısı oluşturmak için aşağıdaki formu kullanın.</p>
    ...
    <form action="<?php echo BASE_URL; ?>panel/blog/save" method="POST" id="blogPostForm" enctype="multipart/form-data">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <button type="button" id="removeThumbnail" class="btn btn-sm btn-danger mt-2" style="display:none;">Görseli
                Kaldır</button>
            <div id="thumbnailPreview" class="mt-2"
                style="max-width: 300px; max-height: 200px; overflow: hidden; display: none;">
                <img id="previewImage" src="#" alt="Görsel Önizleme" style="width: 100%; height: auto; display: block;">
            </div>
        ...
        <div class="mb-4">
            <div class="checkbox-wrapper-46">
                <input type="checkbox" id="is_featured" name="is_featured" class="inp-cbx" value="1" />
        ...
        <div class="mt-4 d-flex gap-3">
            <button type="submit" class="custom-button save-button">
                <span class="text">Yazıyı Kaydet</span>
                <span class="icon">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            removeThumbnailButton.addEventListener('click', function () {
                ...
                removeThumbnailButton.style.display = 'none';
                thumbnailImageUrlInput.value = '';
                // thumbnail_removed bayrağını ekle
            ...
            const editor = new EditorJS({
                ...
                autofocus: true,
                readOnly: false,
                data: {}, // add.php'de veri çekilmez
            });

            // ÖNE ÇIKARILAN BLOG MANTIĞI - ADD.PHP İÇİN
            const isFeaturedCheckbox = document.getElementById('is_featured');
            let confirmationGiven = false; // Onay verildi mi?
            ...
            document.getElementById('blogPostForm').addEventListener('submit', function (event) {
                    ...
                    editor.save().then((outputData) => {
                        ...
                        fetch(this.action, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    alert(data.message);
                                    window.location.href = '<?php echo BASE_URL; ?>panel/blog';
                                } else {
                ...
                // Eğer "Öne Çıkar" seçildiyse ve henüz onay verilmediyse
                if (isFeaturedCheckbox.checked && !confirmationGiven) {
                    // CheckFeatured endpoint'ine kendi ID'mizi göndermeyeceğiz çünkü yeni bir yazıyız
                    fetch('<?php echo BASE_URL; ?>panel/blog/check_featured', {
                        method: 'GET'
                    })
                        ...
                        .then(data => {
                            if (data.is_featured_exists) {
                                ...
                                if (confirm('Mevcut öne çıkarılan blog yazısı ile değiştirilecektir. Onaylıyor musunuz?')) {
                                    confirmationGiven = true; // Onay verildi
                                    submitForm(true); // Formu gönder
                                } else {
                                    // Onay verilmezse checkbox'ı eski haline getir (işaretini kaldır)
                                    isFeaturedCheckbox.checked = false;
                                    console.log('Öne çıkarma işlemi iptal edildi.');
                                }
                            } else {
                                // Zaten öne çıkarılmış yazı yoksa direkt gönder (onay gerekmez)
                                confirmationGiven = true; // Onay verildi sayılır
                                submitForm(true);
                            }
                        })
                        .catch(error => {
                            ...
                            // Hata durumunda yine de formu göndermeye izin ver
                            submitForm(true);
                        });
                } else {
                    // Checkbox seçili değilse veya onay zaten verildiyse direkt formu gönder
                    submitForm(false);
                }
            });
        });
